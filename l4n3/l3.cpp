...
char* concatenateStrings(const char* str1, const char* str2) {
    ...
    char* result = new char[length1 + length2 + 1]; // Allocate memory for new string
    strcpy_s(result, length1 + length2 + 1, str1); // Copy first string
    strcat_s(result, length1 + length2 + 1, str2); // Concatenate second string
    return result;
}
